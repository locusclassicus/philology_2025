...
В отличие от UPOS (Universal Part-Of-Speech), XPOS (Language-specific Part-Of-Speech) -- это теги частей речи, используемые в национальных корпусах.
Форматы тегов и их детализация могут значительно меняться от языка к языку.
